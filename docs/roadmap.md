...
|---------|------------|--------|-------|
| [Accessibility, Routing and UX Polish](plans/archive/accessibility-routing-and-ux-polish-plan.md) | Medium | Completed (2026-09-25) | Skip-to-content link, live route announcer (`aria-live`), explicit 404 Route View (`RouteNotFound`), command-palette search keyboard navigation (`ArrowDown`/`ArrowUp`/`Enter`), dynamic `<meta name="theme-color">` sync |
