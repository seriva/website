...
class SiteConfig {
  constructor({ Title = "", Description = "", Author = "", GithubUsername = "", DefaultRoute = "", DarkTheme = new ThemeColors(), LightTheme = new ThemeColors(), Comments = new CommentsConfig(), EmailJS = new EmailJSConfig(), Search = new SearchConfig(), Social = null, PostsPerPage = 0 } = {}) {
    this.Title = Title;
    this.Description = Description;
    this.Author = Author;
...
function openContact() {
  ...
  contactForm = new ContactState({ ButtonState: "send", ButtonDisabled: false });
  document.documentElement.classList.add("modal-open");
  document.body.classList.add("modal-open");
  let modalEl = document.querySelector("#contact-modal");
  if (modalEl !== null) {
    modalEl.classList.remove("closing");
    modalEl.classList.add("show");
  }
  setTimeout(function() {
    let inp = document.querySelector("#contact-name");
    if (inp !== null) {
      inp.focus();
    }
  }, 50);
...
async function submitContact() {
  ...
  try {
    ...
    render();
    let params = { "title": site.Title, "name": name, "email": email, "message": message };
    ...
    let sendRes = await emailjs.send(site.EmailJS.ServiceId, site.EmailJS.TemplateId, params, site.EmailJS.PublicKey);
    if (sendRes === null && errHappened) {
      return;
    }
...
function DynamicIcon(name, size) {
  return {Mount(___p) {
    switch (name) {
      case "github": {
        (IconGithub(size)).Mount(___p);
      break; }
      case "youtube": {
        (IconYoutube(size)).Mount(___p);
      break; }
      case "linkedin": {
        (IconLinkedin(size)).Mount(___p);
      break; }
      case "download": {
        (IconDownload(size)).Mount(___p);
      break; }
      case "cube": {
        (IconCube(size)).Mount(___p);
      break; }
      case "calendar": {
        (IconCalendar(size)).Mount(___p);
      break; }
      case "search": {
        (IconSearch(size)).Mount(___p);
      break; }
      case "envelope": {
        (IconEnvelope(size)).Mount(___p);
      break; }
      case "expand": {
        (IconExpand(size)).Mount(___p);
      break; }
      case "times": {
        (IconTimes(size)).Mount(___p);
      break; }
      case "arrow-left": {
        (IconArrowLeft(size)).Mount(___p);
      break; }
      case "chevron-down": {
        (IconChevronDown(size)).Mount(___p);
      break; }
      case "chevron-up": {
        (IconChevronUp(size)).Mount(___p);
      break; }
      case "chevron-left": {
        (IconChevronLeft(size)).Mount(___p);
      break; }
      case "chevron-right": {
        (IconChevronRight(size)).Mount(___p);
      break; }
      case "angles-left": {
        (IconAnglesLeft(size)).Mount(___p);
      break; }
      case "angles-right": {
        (IconAnglesRight(size)).Mount(___p);
      break; }
    }
  }};
}
    ...
    if (btn !== null) {
      ...
      switch (action) {
        ...
        case "clear-search":
        {
          ...
          searchQuery = "";
          searchResults = [];
          let clearBtn = document.querySelector("#search-page-clear");
          if (clearBtn !== null) {
...
async function handleRoute() {
  ...
  searchClosing = false;
  contactOpen = false;
  let searchEl = document.querySelector("#search-page");
  if (searchEl !== null) {
  ...
  if (path === "/" || path === "/blog") {
    ...
    currentRoute = "/blog";
    let numStr = path.slice(11);
    let [n, err] = (Number.isNaN(Number(numStr)) ? [0, "invalid syntax"] : [Number(numStr) | 0, null]);
    if (err !== null || n < 1) {
      n = 1;
    }
    blogCurrentPage = n;
...
function initSearch() {
  ...
  let options = { "keys": [{ "name": "title", "weight": 0.4 }, { "name": "description", "weight": 0.3 }, { "name": "tags", "weight": 0.2 }], "threshold": 0.4, "minMatchCharLength": 2 };
  fuseInstance = createFuse(searchItems, options);
}

function performSearch(q) {
  let trimmed = q.trim();
  if (__len(trimmed) < 2 || fuseInstance === null) {
    return [];
  }
