...
- Review follow-ups: removed the unreachable `LoadPending` spinner branches from `blog.templ`, `page.templ`, and `projects.templ` (routes paint once, so they never rendered) along with the orphaned `.loading-spinner` CSS and `general.loading` / `project.loadingReadme` translation keys; extracted `projectFromYAML`, `pageFromYAML`, `sortProjectsByOrder`, and `sortPagesByOrder` from `initData` with unit tests (125 Go tests total); the pre-commit hook now runs `npm run test:gofront` alongside `npm run test:unit`; restored the GoFront Part 2 of `AGENTS.md` after it was accidentally replaced with the pre-rewrite vanilla-JS description.
