...
- Post-rewrite hardening ([plan](docs/plans/archive/post-rewrite-hardening-plan.md)): replaced eleven loose route globals (`currentPost*`, `projectReadme*`, `currentPage*`) with a single `view ViewState` and a `LoadStatus` enum (`LoadReady`/`LoadPending`/`LoadFailed`/`LoadNotFound`) in `src/view.go`, reset on every navigation; split `showPost`/`showProject`/`showPage` into pure, unit-tested resolvers plus async loaders; extracted `resolveRedirect`, `postFromYAML`, and `sortPostsByDate` as pure functions. The loading state is now rendered before the markdown fetch (previously the flag was set but never painted), stale fetches are dropped when another navigation has started (`routeSeq`), and the 404 redirect hash only accepts same-origin paths.
