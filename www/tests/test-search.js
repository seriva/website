QUnit.module("Search Functionality", () => {
	QUnit.test(
		"Search object should exist and have required methods",
		(assert) => {
	...
	QUnit.test("Search should initialize with empty data", (assert) => {
		assert.equal(Search.data.length, 0, "Search data should start empty");
		assert.equal(
			Search.isInitialized,
			false,
			"Search should not be initialized initially",
		);
	});

	QUnit.test("Search constants should be defined", (assert) => {
		assert.equal(
	...
	QUnit.test("Search should handle empty query", (assert) => {
		const results = Search.search("");
		assert.equal(results.length, 0, "Empty query should return no results");
	});

	QUnit.test("Search should handle null/undefined query", (assert) => {
	...
	QUnit.test("Search should respect min characters", (assert) => {
		// Mock some data
		Search.data = [{ title: "Test Project", type: "project" }];
	...
	QUnit.test("Search should limit results to max", (assert) => {
