...
QUnit.module("Simple Tests", () => {
	QUnit.test("Basic utility functions work", (assert) => {
		// Test escapeHtml
	...
	QUnit.test("Template functions exist and work", (assert) => {
		assert.ok(typeof Templates === "object", "Templates object should exist");
		assert.ok(
			typeof Templates.navbar === "function",
			"navbar template should be a function",
		);
		assert.ok(
			typeof Templates.pageLink === "function",
			"pageLink template should be a function",
		);
	...
	QUnit.test("DOM elements exist", (assert) => {
		assert.ok(
	...
	QUnit.test("CSS custom properties are defined", (assert) => {
	...
	QUnit.test("Mobile viewport detection", (assert) => {
	...
	QUnit.test("Template generation works", (assert) => {
		...
		assert.ok(
			pageLink.includes('href="/?page=about"'),
			"Page link should have correct href",
		);
		...
		assert.ok(
			socialLink.includes("https://github.com/test"),
			"Social link should include href",
		);
		assert.ok(
			socialLink.includes("fab fa-github"),
			"Social link should include icon",
		);
	});

	QUnit.test("Error handling works", (assert) => {
		// Test error message template
		const errorMsg = Templates.errorMessage("Test Error", "Test message");
		assert.ok(
			errorMsg.includes("Test Error"),
			"Error message should include title",
		);
		assert.ok(
			errorMsg.includes("Test message"),
	...
	QUnit.test("Loading spinner works", (assert) => {
		const spinner = Templates.loadingSpinner();
		assert.ok(
			spinner.includes("loading-spinner"),
			"Loading spinner should have correct class",
		);
		assert.ok(
			spinner.includes("div"),
			"Loading spinner should be a div element",
		);
	});
});
